...
int global_counter;
pthread_mutex_t global_counter_mutex_lock = PTHREAD_MUTEX_INITIALIZER;
...
int main () 
{
   ...
   return 0;
    ...
    void* count3(void* ID)
    {
        ...
        pthread_mutex_lock(&global_counter_mutex_lock);
        global_counter += thread_counter[(int)ID].value;
        pthread_mutex_unlock(&global_counter_mutex_lock);
